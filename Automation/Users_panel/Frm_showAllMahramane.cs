...
{
    public partial class Frm_showAllMahramane : Form
    {
        automation_systemEntities database = new automation_systemEntities(PublicVariable.MainConnectionString);
        public Frm_showAllMahramane()
        {
            InitializeComponent();
        }

        private void Frm_showAllMahramane_Load(object sender, EventArgs e)
        {
            this.Left = 216;
            this.Top = 142;
        ...
        {
            var query = database.Database.SqlQuery<Vw_recieveLetters>("select * from Vw_RecieveLetters where DraftType=2 and SecurityType = 2 and userId_girande =" + PublicVariable.gUserId + " " + SearchString);
