...
{
    public partial class Frm_showAllNotReadLetters : Form
    {
        automation_systemEntities database = new automation_systemEntities(PublicVariable.MainConnectionString);
        public Frm_showAllNotReadLetters()
        {
            InitializeComponent();
        }

        private void Frm_showAllNotReadLetters_Load(object sender, EventArgs e)
        {
            this.Left = 216;
            this.Top = 142;
        ...
        {
            var query = database.Database.SqlQuery<Vw_recieveLetters>("select * from Vw_RecieveLetters where DraftType=2 and readType = 1 and userId_girande =" + PublicVariable.gUserId + " " + SearchString);
