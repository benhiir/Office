...
{
    public partial class frm_ShowAllnewsforUser : Form
    {
        automation_systemEntities database = new automation_systemEntities(PublicVariable.MainConnectionString);
        public frm_ShowAllnewsforUser()
        {
            InitializeComponent();
        }

        private void frm_ShowAllnewsforUser_Load(object sender, EventArgs e)
        {
            this.Left = 216;
            this.Top = 142;
            persianDateTimePicker_az.Value = DateTime.Now.AddDays(-1);
        ...
        private void dgvShowallnews_DoubleClick(object sender, EventArgs e)
        {
            frm_showMatnNews frm_matn = new frm_showMatnNews();

            frm_matn.lbl_subject.Text = "subject : " + dgvShowallnews.CurrentRow.Cells["subject"].Value.ToString();
            frm_matn.lbl_matn.Text = "Text : " + dgvShowallnews.CurrentRow.Cells["Matn"].Value.ToString();
            frm_matn.ShowDialog();
        }
    }
}
